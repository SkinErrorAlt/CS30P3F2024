...
public class Text
{
	...
	public static <Types> void addButton(Types... Functions) // Makes a easy button that can be made in just a simple function.
	{
		...
		{
			CurrentFunctionNum++;
			int FunctionsAhead = CurrentFunctionNum - Functions.length;
			
			if (Functions.length == 1) 
			{
				Message += "[" + Function + "]: Linked to Unknown";
			}
			else if (Functions.length == 0)
			{
				Console.error("add Button (Console)", "Failed to give button", "Nothing given");
				Message += "[Null] Null";
			}
			else 
			{
				...
				else 
				{
					if (FunctionsAhead > 0) 
					{
						if (Function instanceof Number) 
						{
							if (Functions[CurrentFunctionNum - 1] instanceof Number) 
							{
								Message += " / " + Function;
							}
						}
	...
	public static void addSpace(int... Amounts) // Adds a quick and easy space with adjustable amount. And also allowing for me to just put nothing which will allow for more easier spacing.
	{
		if (Amounts.length == 0) // If nothing is added to the function it will just add a space.
		{
			System.out.println(""); // Adds a space
		}
		else // If more or one is added it will just do the first number given and will just print that number of times.
		{
			for (int i = 0; i < Amounts[0]; i++) // Loops for first amount given.
			{
				System.out.println(""); // Adds a space every loop.
			}
		}
	}
